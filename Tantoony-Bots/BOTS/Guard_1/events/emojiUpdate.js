...
module.exports = class {
    ...
    async run(prev, emoji) {
        ...
        if (systemf && (systemf.type === "update") && (systemf.effect === "emoji")) {
            if (systemf.count > 0) {
                await izin.updateOne({ _id: mal.user.id }, { $inc: { count: -1 } });
                return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${prev.name}** emojisi ${mal} tarafından başarıyla ${emoji.name} olarak düzenlendi`));
            } else {
                await izin.deleteOne({ _id: mal.user.id });
            };
        };
        if (utiller.get("kkv").value().some(id => logs.executor.id === id)) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${prev.name}** emojisi ${mal} tarafından başarıyla ${emoji.name} olarak düzenlendi`));
        if (mal.roles.cache.has(roller.get("root").value())) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla düzenlendi`));
        if (mal.roles.cache.has(roller.get("owner").value())) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla düzenlendi`));
        let rolz = [];
        let rolidleri = [];
        ...
        if (!system) {
            try {
                let doggy = await jailed({ _id: mal.user.id, sebep: "KURAL DIŞI EYLEM - EMOJİ DÜZENLEME", executor: emoji.guild.owner.id, rolz: rolz, created: new Date() });
                await doggy.save();
            } catch (error) {
                if (error.code !== 5904) {
                    throw error;
        ...
        const embedd = embed.setTitle(`Jaile Gönderildi`).setDescription(`${emojis.jailed} ${mal} kişisi ${this.client.owner} tarafından cezalıya atıldı`)
            .addField("Sebep:", "KURAL DIŞI EYLEM - EMOJİ DÜZENLEME", true).addField("Süre", `**Perma**`, true).setThumbnail(mal.user.displayAvatarURL({ dynamic: true }))
        ...
        const obje = {
            ...
            reason: "KURAL DIŞI EYLEM - EMOJİ DÜZENLEME"
        }
        let invarray = [];
        ...
        emoji.edit({ name: prev.name });
        lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("warn").value()} ÖNEMLİ UYARI!!!! ${logs.executor} bir emoji yenilemeye çalıştı. Yenilenen emoji: ${emoji.name}`));
