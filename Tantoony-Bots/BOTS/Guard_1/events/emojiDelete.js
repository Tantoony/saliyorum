...
module.exports = class {
    ...
    async run(emoji) {
        ...
        if (systemf && (systemf.type === "delete") && (systemf.effect === "emoji")) {
            if (systemf.count > 0) {
                await izin.updateOne({ _id: mal.user.id }, { $inc: { count: -1 } });
                return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla silindi`));
            } else {
                await izin.deleteOne({ _id: mal.user.id });
            };
        };
        if (utiller.get("kkv").value().some(id => logs.executor.id === id)) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla silindi`));
        if (mal.roles.cache.has(roller.get("root").value())) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla silindi`));
        if (mal.roles.cache.has(roller.get("owner").value())) return lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("idea").value()} **${emoji.name}** emojisi ${mal} tarafından başarıyla silindi`));
        let rolz = [];
        let rolidleri = [];
        ...
        if (!system) {
            try {
                let doggy = await jailed({ _id: mal.user.id, sebep: "KURAL DIŞI EYLEM - EMOJİ SİLME", executor: emoji.guild.owner.id, rolz: rolz, created: new Date() });
                await doggy.save();
            } catch (error) {
                if (error.code !== 5904) {
                    throw error;
        ...
        const embedd = embed.setTitle(`Jaile Gönderildi`).setDescription(`${emojis.jailed} ${mal} kişisi ${this.client.owner} tarafından cezalıya atıldı`)
            .addField("Sebep:", "KURAL DIŞI EYLEM - EMOJİ SİLME", true).addField("Süre", `**Perma**`, true).setThumbnail(mal.user.displayAvatarURL({ dynamic: true }))
        ...
        const obje = {
            ...
            reason: "KURAL DIŞI EYLEM - EMOJİ SİLME"
        }
        let invarray = [];
        ...
        lokal.send(new Discord.MessageEmbed().setDescription(`${emojiler.get("warn").value()} ÖNEMLİ UYARI!!!! ${logs.executor} bir emoji silmeye çalıştı. Silinen emoji: ${emoji.name}`));
    }
} 
